...
def do_inference(model, file, reshape, **kwargs):
...
def transformer_inshape(inp):
    inp = inp[:, 0:999, 0:999]
    batched = rearrange(inp, "c (b1 h) (b2 w) -> (b1 b2) c h w", h=27, w=27, b1=37, b2=37)
    return batched

def transformer_outshape(inp):
    unbatched = rearrange(inp, "(b1 b2) h w -> (b1 h) (b2 w)", h=27, w=27, b1=37, b2=37)
    return unbatched
